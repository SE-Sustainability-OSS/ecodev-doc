...
!!! warning
    The `/stats/*` routes are part of the **FastAPI** backend. Set `CF_TOOL_BASE_URL` etc.
    to the producer's `fastapi_url` in `.env`. The Docker service name defaults only work
    when both apps share the same compose network.

---

## 3. Granularity: hour vs. month

The ingest has two granularities; both must be consistent across the fetch, delete, and
upsert calls so the delete scope matches the ingest scope:
...
!!! tip
    Pass the same `granularity` to `fetch_activities`, `delete_lookback_activities`, and
    `upsert_remote_activities`. If they diverge, the delete removes rows for a different grain
    than the fetch produced, breaking idempotency.
